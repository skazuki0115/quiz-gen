...
export function appendHistory(entry) {
  ...
  db.prepare(
    `INSERT INTO quiz_history
      (id, prompt, difficulty, question, choices, answer, explanation, pdf_id, chapter_id, chapter_title, chunk_ids, created_at)
     VALUES
      (@id, @prompt, @difficulty, @question, @choices, @answer, @explanation, @pdf_id, @chapter_id, @chapter_title, @chunk_ids, @created_at)`
  ).run({
    ...
    pdf_id: entry.pdfId ?? null,
    chapter_id: entry.chapterId ?? null,
    chapter_title: entry.chapterTitle ?? null,
    chunk_ids: Array.isArray(entry.chunkIds) ? JSON.stringify(entry.chunkIds) : JSON.stringify([]),
...
export function readHistory({ limit, order = 'ASC' } = {}) {
  ...
  let query =
    'SELECT id, prompt, difficulty, question, choices, answer, explanation, pdf_id, chapter_id, chapter_title, chunk_ids, created_at FROM quiz_history ORDER BY datetime(created_at) ' +
    clause;
  ...
  return rows.map(row => ({
    ...
    chunkIds: parseChoices(row.chunk_ids ?? '[]'),
